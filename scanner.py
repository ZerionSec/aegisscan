...
SUBDOMAIN_LIST = [
    ...
    "staging", "blog", "docs", "support", "portal", "app",
    "secure", "vpn", "git", "jenkins", "grafana", "kibana",
    "elastic", "redis", "mysql",
]
...
TECH_SIGNATURES = {
    ...
    "Drupal": ["/misc/drupal.js", "Drupal"],
    "Joomla": ["/media/system/js/", "Joomla!"],
    "Laravel": ["laravel_session"],
    "Django": ["/static/admin/", "csrftoken"],
    "Nginx": ["server: nginx"],
...
SENSITIVE_PATH_MARKERS = (
    ".env", ".git/config", ".git/", "aws/credentials",
    "wp-config", ".htpasswd", ".htaccess",
)

NON_CRITICAL_PATH_MARKERS = (
    "robots.txt", ".gitignore", "package.json", "composer.json",
)
...
class WebSecurityScanner:
    ...
    def check_headers_and_cookies(self):
        try:
            response = self._request("GET", self.target, allow_redirects=True)
            ...
            for header in SECURITY_HEADERS:
                ...
                self.results["headers"][header] = value if value else "MISSING"
            ...
            if acao == "*":
                self.results["cors"] = {
                    ...
                    "note": (
                        ...
                        "Impact depends on the resources and authentication model."
                    ),
                }
            elif acao:
                self.results["cors"] = {
                ...
                self.results["cors"] = {"status": "MISSING"}
    ...
    def _evaluate_csp(self, csp: str) -> Dict[str, Any]:
        ...
        if "default-src *" in normalized:
            issues.append("default-src uses a wildcard.")
        if "default-src" not in normalized and "script-src" not in normalized:
            issues.append("No default-src or script-src directive detected.")
            ...
            with socket.create_connection(
                ...
                with context.wrap_socket(
                    ...
                    self.results["tls"] = {
                        ...
                        "secure_protocol": (
                            tls_socket.version() in ("TLSv1.2", "TLSv1.3")
                        ),
                    }
    ...
    def check_subdomains(self):
        ...
        def resolve(subdomain):
            hostname = f"{subdomain}.{self.hostname}"
            try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            futures = [
                executor.submit(resolve, subdomain)
    ...
    def check_technologies(self):
        ...
        for technology, signatures in TECH_SIGNATURES.items():
            for signature in signatures:
                ...
                if signature.startswith("server:"):
                    ...
                    expected = signature.replace("server:", "").strip()
                    if expected in server:
                        detected.append(technology)
                        break
                elif signature in html_lower or signature in str(headers_lower):
                    detected.append(technology)
                    break
        ...
        self.results["technologies"] = sorted(set(detected))
    ...
    def check_secrets(self):
        ...
        for secret_type, pattern in SECRET_PATTERNS.items():
            for match in pattern.finditer(self.raw_html):
                full_match = match.group(0)
                if match.groups():
                    secret_value = match.group(1)
                    masked = self._mask_secret(secret_value)
                    display = full_match.replace(secret_value, masked)
                else:
                    display = self._mask_secret(full_match)
                findings.append({"type": secret_type, "evidence": display})

        unique = []
        seen = set()
        for item in findings:
            key = (item["type"], item["evidence"])
            if key not in seen:
                seen.add(key)
                unique.append(item)
    ...
    def check_exposed_files(self):
        ...
        def check_path(path):
            try:
                response = self._request(
                    "GET", self.target + path, allow_redirects=False
                )
                return {
                    "path": path,
        ...
        self.results["exposed_files"] = sorted(found, key=lambda x: x["path"])
        ...
        try:
            ...
            if "index of /" in body_lower or "parent directory" in body_lower:
                self.add_finding(
                    ...
                    confidence="MEDIUM",
                    score=5,
                    evidence="Directory listing indicators were detected in the response.",
                    remediation="Disable directory indexing unless it is explicitly required.",
                )
        except requests.RequestException:
            pass

        server = self.resp_headers.get("Server", "")
        if server:
            self.add_finding(
                ...
                remediation="Minimize unnecessary server version disclosure.",
            )

        if self.parsed.scheme == "http":
            try:
                response = self._request("GET", self.target, allow_redirects=False)
                ...
                if not (
                    ...
                    self.add_finding(
                        ...
                        remediation="Redirect HTTP requests to HTTPS and enforce secure transport.",
                    )
            except requests.RequestException:
                pass
    ...
    def _evaluate_security_headers(self):
        ...
        if headers.get("Content-Security-Policy") == "MISSING":
            self.add_finding(
                ...
                remediation="Deploy a restrictive CSP appropriate to the application's resources.",
            )
        ...
        if headers.get("X-Frame-Options") == "MISSING":
            self.add_finding(
                ...
                confidence="HIGH",
                score=5,
                evidence="X-Frame-Options was not present.",
                remediation="Use X-Frame-Options or an appropriate CSP frame-ancestors policy.",
            )
        ...
        if headers.get("X-Content-Type-Options") == "MISSING":
            self.add_finding(
                ...
                confidence="HIGH",
                score=2,
                evidence="X-Content-Type-Options was not present.",
        ...
        if headers.get("Referrer-Policy") == "MISSING":
            self.add_finding(
                ...
                confidence="HIGH",
                score=2,
                evidence="Referrer-Policy was not present.",
                remediation="Configure an appropriate Referrer-Policy for the application.",
            )

        if headers.get("Permissions-Policy") == "MISSING":
            self.add_finding(
                ...
                confidence="MEDIUM",
                score=2,
                evidence="Permissions-Policy was not present.",
                remediation="Restrict unnecessary browser features through Permissions-Policy.",
            )
    ...
    def _evaluate_cookies(self):
        for cookie in self.results["cookies"]:
            ...
            if str(cookie["samesite"]).lower() in ("", "not set", "none"):
            ...
            self.add_finding(
                ...
                confidence="HIGH",
                score=5,
                evidence=f"Missing or weak attributes: {', '.join(problems)}",
                remediation=(
    ...
    def _evaluate_csp_finding(self):
        ...
        if not csp or csp.get("status") == "MISSING":
            return
        ...
        self.add_finding(
            ...
            remediation=(
                "Review CSP directives and minimize unsafe-inline, "
    ...
    def _evaluate_cors_finding(self):
        ...
        self.add_finding(
            ...
            remediation=(
                "Use an explicit trusted origin list when cross-origin "
                "access needs restriction. Verify the affected resources "
        ...
        for item in self.results["exposed_files"]:
            ...
            if any(marker in path for marker in SENSITIVE_PATH_MARKERS):
                self.add_finding(
                    ...
                    confidence="HIGH",
                    score=20,
                    evidence=f"{path} returned HTTP 200 ({item['size']} bytes).",
                    remediation=(
                        "Remove the file from the web root or deny web access. "
                        "Rotate exposed credentials if sensitive material was accessible."
                    ),
                )
            elif any(marker in path for marker in NON_CRITICAL_PATH_MARKERS):
                self.add_finding(
                    ...
                    remediation="Confirm that the resource is intentionally publicly accessible.",
                )
    ...
    def _evaluate_ports(self):
        for port in self.results["open_ports"]:
            ...
            self.add_finding(
                ...
                confidence="HIGH",
                score=5,
                evidence=f"TCP port {port} ({service}) accepted a connection.",
                remediation=(
                    "Restrict administrative/database services to trusted "
                    "networks or VPN access where appropriate."
                ),
            )
    ...
    def _evaluate_tls(self):
        ...
        if tls.get("status") == "ERROR":
            self.add_finding(
                ...
                confidence="LOW",
                score=5,
                evidence=tls.get("error", "Unknown TLS error"),
                remediation="Verify TLS configuration and certificate compatibility.",
            )
            return
        ...
        if not tls.get("secure_protocol", False):
            self.add_finding(
                ...
                confidence="HIGH",
                score=10,
                evidence=f"Negotiated protocol: {tls.get('version', 'unknown')}",
                remediation="Disable obsolete TLS protocols and support TLS 1.2 or TLS 1.3.",
            )

    def calculate_risk_score(self):
        findings = self.results["findings"]
        score = sum(int(finding.get("score", 0)) for finding in findings)
        score = min(score, 100)
    ...
    def scan(self):
        ...
        if self.results["secrets_found"]:
            evidence = ", ".join(
                item["type"] for item in self.results["secrets_found"]
            )
            self.add_finding(
    ...
    def report_text(self) -> str:
        out = []
        out.append("=" * 78)
        out.append(f"  AegisScan v{VERSION}")
        out.append(f"  Target: {self.target}")
        out.append(f"  Risk Score: {self.results['risk_score']}/100")
        out.append(f"  Risk Level: {self.results['risk_level']}")
        out.append(f"  Timestamp: {self.results['timestamp']}")
        out.append("=" * 78)

        counts = self.results.get("severity_counts", {})
        out.append("\n[+] Finding Summary")
        for severity in ("CRITICAL", "HIGH", "MEDIUM", "LOW", "INFO"):
            out.append(f"    {severity}: {counts.get(severity, 0)}")
        ...
        if self.results["findings"]:
            ...
            for finding in sorted_findings:
                out.append(
                    f"    {finding['id']}: +{finding['score']} "
                    f"[{finding['severity']}] {finding['title']}"
                )
        else:
            out.append("    No findings contributed to the score.")
        ...
        if not self.results["findings"]:
            ...
            for finding in self.results["findings"]:
                ...
                out.append(f"    Score: +{finding['score']}")
        ...
        if self.results["cookies"]:
            ...
            out.append("    No response cookies detected.")

        out.append("\n[+] CORS")
        out.append(f"    {self.results['cors']}")

        out.append("\n[+] Discovered Subdomains")
        if self.results["subdomains"]:
            for subdomain in self.results["subdomains"]:
                out.append(f"    {subdomain}")
        else:
        ...
        out.append(
            "    " + (
                ", ".join(self.results["technologies"])
        ...
        out.append(
            "    " + (
                ", ".join(map(str, self.results["open_ports"]))
        ...
        out.append("\n[+] Checked Paths")
        for item in self.results["exposed_files"]:
            state = "ACCESSIBLE" if item["accessible"] else "PROTECTED/BLOCKED"
            out.append(f"    {item['path']} -> {item['status']} ({state})")

        out.append("\n[+] Potential Secrets")
        if self.results["secrets_found"]:
            for secret in self.results["secrets_found"]:
                out.append(f"    {secret['type']}: {secret['evidence']}")
        else:
...
def complete_html_report(scanner: WebSecurityScanner) -> str:
    def esc(value):
    ...
    if not finding_rows:
        finding_rows = """
        <tr><td colspan="7">No findings detected.</td></tr>
        """

    header_rows = ""
    for key, value in scanner.results["headers"].items():
        header_rows += f"""
        <tr><td>{esc(key)}</td><td>{esc(value)}</td></tr>
        """

    cookie_rows = ""
    for cookie in scanner.results["cookies"]:
        cookie_rows += f"""
        <tr>
    ...
    file_rows = ""
    for item in scanner.results["exposed_files"]:
        state = "Accessible" if item["accessible"] else "Protected/Blocked"
        file_rows += f"""
        <tr>
            <td>{esc(item["path"])}</td>
    ...
    if not file_rows:
        file_rows = """
        <tr><td colspan="4">No relevant paths returned 200/403.</td></tr>
        """

    subdomains = "<br>".join(
        esc(x) for x in scanner.results["subdomains"]
    ) or "None"

    technologies = ", ".join(
        esc(x) for x in scanner.results["technologies"]
    ) or "None"

    ports = ", ".join(map(str, scanner.results["open_ports"])) or "None"
...
<title>AegisScan — Web Security Assessment</title>
<style>
body {{ font-family: Arial, sans-serif; background: #f3f4f6; margin: 0; padding: 30px; color: #1f2937; }}
.container {{ max-width: 1200px; margin: auto; background: white; padding: 30px; border-radius: 12px; }}
h1 {{ margin-top: 0; }}
h2 {{ margin-top: 35px; }}
.score {{ display: inline-block; padding: 15px 25px; border-radius: 10px; color: white; font-size: 30px; font-weight: bold; }}
...
.note {{ background: #f3f4f6; padding: 12px; border-radius: 8px; margin-top: 15px; }}
</style>
</head>
<body>
<div class="container">
<h1>🛡️ AegisScan — Web Security Assessment Report</h1>
<p>
<strong>Scanner:</strong> AegisScan v{esc(VERSION)}<br>
<strong>Target:</strong> {esc(scanner.target)}<br>
<strong>Timestamp:</strong> {esc(scanner.results["timestamp"])}
</p>

<h2>📊 Risk Score</h2>
<div class="score {score_class}">{score}/100</div>
<p><strong>Risk Level:</strong> {esc(scanner.results["risk_level"])}</p>
<div class="note">This is a project-specific risk indicator and is not a CVSS score.</div>

<h2>⚠️ Findings</h2>
<table>
<tr><th>ID</th><th>Finding</th><th>Severity</th><th>Confidence</th><th>Score</th><th>Evidence</th><th>Remediation</th></tr>
{finding_rows}
</table>

<h2>📋 Security Headers</h2>
<table><tr><th>Header</th><th>Value</th></tr>{header_rows}</table>
...
{''.join(f"<tr><td>{esc(k)}</td><td>{esc(v)}</td></tr>" for k, v in scanner.results["tls"].items())}
</table>

<h2>🌐 CORS</h2>
<p>{esc(scanner.results["cors"])}</p>
...
<h2>📂 Checked Paths</h2>
<table>
<tr><th>Path</th><th>Status</th><th>Size</th><th>Classification</th></tr>
{file_rows}
</table>

<h2>🔑 Potential Secrets</h2>
{''.join(f"<p><strong>{esc(x['type'])}:</strong> {esc(x['evidence'])}</p>" for x in scanner.results["secrets_found"]) or "<p>None detected.</p>"}

<p style="margin-top:40px;color:#6b7280;">Generated by AegisScan v{esc(VERSION)}</p>
</div>
</body>
</html>
"""
...
def scan_multiple(
    ...
    for target in targets:
        ...
        try:
            ...
            if output_format == "json":
                json_reports.append(scanner.results)
            elif output_format == "html":
                reports.append(complete_html_report(scanner))
            else:
    ...
    if output_format == "text":
        if output_file:
            print("[!] Text mode prints to console. Use --json or --html for file output.")
        return

    if output_format == "json":
        final_output = json.dumps(json_reports, indent=2, ensure_ascii=False)
    else:
        if len(reports) == 1:
            ...
            for index, report in enumerate(reports, start=1):
                body += f"<hr><h1>Target {index}</h1>{report}"
            final_output = f"""<!DOCTYPE html>
<html><head><meta charset="UTF-8"><title>Multi-Target Security Assessment</title></head>
<body>{body}</body></html>"""
...
def main():
    parser = argparse.ArgumentParser(
        description="AegisScan v3.2 - Defensive Web Security Scanner"
    )
    parser.add_argument("targets", nargs="+", help="Authorized target URL(s)")
    ...
    parser.add_argument(
        ...
        help=f"Delay between HTTP requests (default: {DEFAULT_RATE_LIMIT}s)",
    )
    parser.add_argument(
        "--timeout",
        type=int,
    ...
    if args.json and args.html:
        parser.error("Use either --json or --html, not both.")
